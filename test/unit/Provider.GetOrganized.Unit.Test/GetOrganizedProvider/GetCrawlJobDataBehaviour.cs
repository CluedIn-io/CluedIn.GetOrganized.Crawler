...
{
    public class GetCrawlJobDataBehaviour : GetOrganizedProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(GetOrganizedConstants.KeyName.ApiKey, nameof(GetOrganizedCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task GetOrganizedCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.GetOrganized.Core.GetOrganizedCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
