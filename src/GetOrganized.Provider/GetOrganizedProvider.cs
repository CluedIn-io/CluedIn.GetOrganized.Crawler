...
{
    public class GetOrganizedProvider : ProviderBase, IExtendedProviderMetadata
    {
        private readonly IGetOrganizedClientFactory _getorganizedClientFactory;

        public GetOrganizedProvider([NotNull] ApplicationContext appContext, IGetOrganizedClientFactory getorganizedClientFactory)
            : base(appContext, GetOrganizedConstants.CreateProviderMetadata())
        {
            _getorganizedClientFactory = getorganizedClientFactory;
        }
            ...
            Guid providerDefinitionId)
        {
            ...
            if (jobData is GetOrganizedCrawlJobData getorganizedCrawlJobData)
            {
                ...
                dictionary.Add(GetOrganizedConstants.KeyName.ApiKey, getorganizedCrawlJobData.ApiKey);
            }
        ...
        public override async Task<AccountInformation> GetAccountInformation(ExecutionContext context, [NotNull] CrawlJobData jobData, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            var client = _getorganizedClientFactory.CreateNew(getorganizedCrawlJobData);
        ...
        public string Icon => GetOrganizedConstants.IconResourceName;
        public string Domain { get; } = GetOrganizedConstants.Uri;
        public string About { get; } = GetOrganizedConstants.CrawlerDescription;
        public AuthMethods AuthMethods { get; } = GetOrganizedConstants.AuthMethods;
        public IEnumerable<Control> Properties => null;
        public string ServiceType { get; } = JsonConvert.SerializeObject(GetOrganizedConstants.ServiceType);
        public string Aliases { get; } = JsonConvert.SerializeObject(GetOrganizedConstants.Aliases);
        public Guide Guide { get; set; } = new Guide
        {
            Instructions = GetOrganizedConstants.Instructions,
            Value = new List<string> { GetOrganizedConstants.CrawlerDescription },
            Details = GetOrganizedConstants.Details

        };

        public string Details { get; set; } = GetOrganizedConstants.Details;
        public string Category { get; set; } = GetOrganizedConstants.Category;
        public new IntegrationType Type { get; set; } = GetOrganizedConstants.Type;
    }
}
